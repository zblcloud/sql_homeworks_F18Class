--create table HR (employee_id int, first_name varchar(50), last_name varchar(50), email varchar(50), phone_number varchar(50), hire_date date, job_id varchar(20), salary float, commission_pct float, manager_id int, department_id int)
...
--select first_name, last_name, salary from HR
--where salary < 6000

--select first_name, last_name, department_id, salary from HR
--where salary > 8000

--select first_name, last_name, department_id from HR
...
--WHERE first_name NOT LIKE '%M%'  ORDER BY department_id; -- full name haqida ma'lumot olish 
...
--select CONCAT(first_name,' ', last_name) as full_name, salary from HR
--where commission_pct is NULL

--select concat(first_name, ' ', last_name) as full_name, concat(phone_number, ' ', email) as contact_details, salary from HR
...
--select concat(first_name, '', last_name) as full_name, job_id, hire_date from HR
--where hire_date between '2007-11-05' and '2009-07-05'

--select concat(first_name, '', last_name) as full_name, department_id from HR
--where department_id = 70 or department_id = 90

--select concat(first_name, '', last_name) as full_name, salary, manager_id from HR
--where manager_id > 0

--select * from HR
--where hire_date < '2002-06-21'

--select first_name, last_name, email, salary, manager_id from HR
...
--hire_date, commission_pct, email, replace(phone_number, '.', '-') as formatted_phone, salary from HR
...
--select employee_id, first_name, job_id, department_id from HR
--where department_id not in (50, 30, 80)

--select employee_id, first_name, job_id, department_id from HR
--where department_id in (30, 40, 90)

--create table job (employee_id int, start_date date, end_date date, job_id varchar(20), department_id int)
...
--select employee_id from job 
--group by employee_id having count(*) >=2;

--select job_id, count(*) as job_count, sum(salary) as job_sum, max(salary)-min(salary) as salary_difference from hr
--group by job_id;

--SELECT job_id FROM HR WHERE hire_date >300 GROUP BY job_id HAVING COUNT(*)>=2; --- 24 vazifani so'rash

--create table locations (location_id int, street_address varchar(40), postal_code int, city varchar(30), state_province varchar(30), country_id varchar(20))
--alter table locations 
--alter column postal_code varchar(30);
...
--(1800, '147 Spadina Ave                         ', 'M5V_2L7   ', 'Toronto            ', 'Ontario         ', 'CA'),
--(1900, '6092 Boxwood St                         ', 'YSW_9T2   ', 'Whitehorse         ', 'Yukon           ', 'CA'),
--(2000, '40-5-12 Laogianggen                     ', '190518    ', 'Beijing            ', '                ', 'CN'),
--(2100, '1298 Vileparle (E)                      ', '490231    ', 'Bombay             ', 'Maharashtra     ', 'IN'),
--(2200, '12-98 Victoria Street                   ', '2901      ', 'Sydney             ', 'New South Wales ', 'AU'),
--(2300, '198 Clementi North                      ', '540198    ', 'Singapore          ', '                ', 'SG'),
--(2400, '8204 Arthur St                          ', '   00     ', 'London             ', '                ', 'UK'),
--(2500, 'Magdalen Centre, The Oxford Science Park', 'OX9_9ZB   ', 'Oxford             ', 'Oxford          ', 'UK'),
--(2600, '9702 Chester Road                       ', '9629850293', 'Stretford          ', 'Manchester      ', 'UK'),
--(2700, 'Schwanthalerstr. 7031                   ', '80925     ', 'Munich             ', 'Bavaria         ', 'DE'),
--(2800, 'Rua Frei Caneca 1360                    ', '01307-002 ', 'Sao Paulo          ', 'Sao Paulo       ', 'BR'),
--(2900, '20 Rue des Corps-Saints                 ', '1730      ', 'Geneva             ', 'Geneve          ', 'CH'),
--(3000, 'Murtenstrasse 921                       ', '3095      ', 'Bern               ', 'BE              ', 'CH'),
--(3100, 'Pieter Breughelstraat 837               ', '3029-SK   ' , 'Utrecht            ', 'Utrecht         ', 'NL'),
--(3200, 'Mariano Escobedo 9991                   ', '11932     ', 'Mexico City        ', 'Distrito Federal', 'MX');

--select country_id, count(*) as number_of_cities from locations
--group by country_id 

--select manager_id, count(*) as number_of_employees from HR
--group by manager_Id

--create table jobs (job_id varchar(30), job_title varchar(30), min_salary int, max_salary int);
--alter table jobs
...
--select first_name, last_name, hire_date from HR
--where job_id = 'SA_REP' or job_id = 'SA_MAN';

--select department_id, avg(salary) as average_salary from HR
--where commission_pct IS not null
--group by department_id

--select distinct department_id from HR 
...
--select job_id, avg(salary) as average_salary from HR 
--group by job_id 
--having avg(salary) > 8000

--select job_title, max_salary-min_salary as saldo from jobs
--where max_salary between 12000 and 18000;

--select first_name, last_name from HR 
